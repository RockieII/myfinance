...
function draw(container) {
  ...
  container.innerHTML = `
    <div class="flex-between mb-12">
      <span class="fs-12 text-dim">${filtered.length === 1 ? t('1 transaction') : t('{n} transactions', { n: filtered.length })}</span>
      <button class="btn btn-primary btn-sm" id="add-tx-btn">${t('+ Add')}</button>
    </div>
    ...
    <div class="filter-row mb-12">
      ...
      <select id="f-type" class="form-control filter-select">
        ...
        <option value="income" ${filters.type === 'income' ? 'selected' : ''}>${t('Income')}</option>
        <option value="expense" ${filters.type === 'expense' ? 'selected' : ''}>${t('Expense')}</option>
      </select>
      <select id="f-category" class="form-control filter-select">
        <option value="">${t('All categories')}</option>
        ${categories.map(c => `<option value="${c.id}" ${filters.category_id === c.id ? 'selected' : ''}>${c.name} (${t(c.type)})</option>`).join('')}
      </select>
...
function renderPage(container, filtered) {
  ...
  const paint = (n) => {
    ...
    region.innerHTML = filtered.length
      ...
      : `<div class="empty">${t('No transactions found.')}</div>`;
    return pages;
  };
  ...
  region.querySelectorAll('[data-edit-tx]').forEach(btn => {
    btn.addEventListener('click', () => {
      const tx = transactions.find(x => x.id === btn.dataset.editTx);
  ...
  return `
    <div class="row" style="padding:11px 16px">
      ...
      <div class="flex gap-8" style="flex-shrink:0;margin-left:8px">
        <button data-edit-tx="${tx.id}" class="btn-icon" title="${t('Edit')}"><i class="ph ph-pencil-simple"></i></button>
        <button data-delete-tx="${tx.id}" class="btn-icon" title="${t('Delete')}"><i class="ph ph-trash"></i></button>
      </div>
    </div>
  `;
}
...
function openTxForm(container, editingId, tx) {
  ...
  const { el, close } = openSheet(`
    <h3>${editingId ? t('Edit Transaction') : t('New Transaction')}</h3>
    <form id="tx-form">
      <div class="form-group">
        <label>${t('Type')}</label>
        <select id="tx-type" class="form-control">
          <option value="expense" ${currentType === 'expense' ? 'selected' : ''}>${t('Expense')}</option>
          <option value="income" ${currentType === 'income' ? 'selected' : ''}>${t('Income')}</option>
        </select>
      </div>
      <div class="form-group">
        <label>${t('Amount')}</label>
        <input id="tx-amount" type="number" step="0.01" min="0.01" class="form-control" value="${tx.amount || ''}" placeholder="${t('0.00')}" required>
      </div>
      <div class="form-group">
      ...
      <div class="form-group">
        ...
        <input id="tx-desc" class="form-control" value="${tx.description || ''}" placeholder="${t('Optional note')}">
      </div>
      <div class="flex gap-8">
        <button type="submit" class="btn btn-primary">${editingId ? t('Save') : t('Add')}</button>
        <button type="button" class="btn btn-outline" id="tx-cancel">${t('Cancel')}</button>
      </div>
    </form>
  `);
...
function applyFilters(txs) {
  return txs.filter(x => {
    if (filters.month && !x.date.startsWith(filters.month)) return false;
    if (filters.type && x.type !== filters.type) return false;
    if (filters.category_id && x.category_id !== filters.category_id) return false;
    if (filters.account_id && x.account_id !== filters.account_id) return false;
    return true;
  });
}

function getUniqueMonths(txs) {
  return [...new Set(txs.map(x => x.date.slice(0, 7)))].sort().reverse();
}
