const CACHE_NAME = 'myfinance-v12';

const SHELL = [
  './',
  './index.html',
  './manifest.webmanifest',
  './css/main.css',
  './css/grid.css',
  './css/library.css',
  './css/personalize.css',
  './js/app.js',
  './js/config.js',
  './js/db.js',
  './js/auth.js',
  './js/format.js',
  './js/sheet.js',
  './js/profiles.js',
  './js/icons.js',
  './js/i18n.js',
  './js/nav.js',
  './js/seed-data.js',
  './js/views/data.js',
  './js/views/accounts.js',
  './js/dashboards/registry.js',
  './js/dashboards/engine.js',
  './js/dashboards/context.js',
  './js/dashboards/themes.js',
  './js/dashboards/prebuilt.js',
  './js/dashboards/library.js',
  './js/dashboards/page-gallery.js',
  './js/dashboards/personalize.js',
  './js/dashboards/grid-dnd.js',
  './js/dashboards/demo-ctx.js',
  './js/views/dashboards.js',
  './js/views/transactions.js',
  './js/views/stocks.js',
  './js/views/categories.js',
  './js/views/settings.js',
  './js/views/help.js',
  './icons/icon.svg',
];

self.addEventListener('install', event => {
  event.waitUntil(
    caches.open(CACHE_NAME).then(cache => cache.addAll(SHELL))
  );
  self.skipWaiting();
});

self.addEventListener('activate', event => {
  event.waitUntil(
    caches.keys().then(keys =>
      Promise.all(keys.filter(k => k !== CACHE_NAME).map(k => caches.delete(k)))
    )
  );
  self.clients.claim();
});

self.addEventListener('fetch', event => {
  const url = new URL(event.request.url);

  // Network-first for CDN, Supabase API, and Finnhub API
  if (url.host !== location.host || url.host.includes('supabase') || url.host.includes('finnhub')) {
    event.respondWith(
      fetch(event.request).catch(() => caches.match(event.request))
    );
    return;
  }

  // Cache-first for app shell
  event.respondWith(
    caches.match(event.request).then(cached => cached || fetch(event.request))
  );
});
